...
if [ $DISPLAY_INSTALL_STATUS -eq 0 ]; then
  ...
  echo "Starting Burp Suite Community Edition and Google Chrome..."
  export DISPLAY=:20
  sudo -u ${CHROME_REMOTE_USER_NAME} DISPLAY=:20 dbus-launch /opt/BurpSuiteCommunity/BurpSuiteCommunity &
  sudo -u ${CHROME_REMOTE_USER_NAME} DISPLAY=:20 dbus-launch google-chrome &
  echo "Burp Suite Community Edition and Google Chrome started."  
 else
